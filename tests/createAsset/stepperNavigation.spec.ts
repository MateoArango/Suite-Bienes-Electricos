...
test.describe('Crear Activo - P1 Stepper Navigation', () => {
  ...
  test('16. Field counter tracks all filled fields independently from required validation', async ({ page }) => {
    ...
    await expect(createAsset.stepper).toContainText('Campos1 de 13');
    await expect(createAsset.nextBtn).toBeDisabled();
    ...
    await expect(createAsset.stepper).toContainText('Campos3 de 13');  // Dane Code is automatically filled
    await expect(createAsset.nextBtn).toBeDisabled();
    ...
    await expect(createAsset.stepper).toContainText('Campos4 de 13');
    await expect(createAsset.nextBtn).toBeDisabled();
    ...
    await expect(createAsset.stepper).toContainText('Campos5 de 13');
    ...
    await expect(createAsset.stepper).toContainText('Campos6 de 13');
    await expect(createAsset.nextBtn).toBeEnabled();
  });
  ...
  test('17. User can navigate back without losing Location values', async ({ page }) => {
    ...
    await createAsset.backBtn.click();
    await page.getByTestId('activesCreateDepartamento').waitFor({ state: 'visible' });
